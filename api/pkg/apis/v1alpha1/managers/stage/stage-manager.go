...
func (s *StageManager) traceValue(ctx context.Context, v interface{}, namespace string, inputs map[string]interface{}, triggers map[string]interface{}, outputs map[string]map[string]interface{}) (interface{}, error) {
	...
	case string:
		...
		log.InfofCtx(ctx, " M (Stage): Context: %+v", *s.Context)
		if s.Context != nil {
			log.InfofCtx(ctx, " M (Stage): VencorContext: %+v", *s.Context.VencorContext)
			if s.Context.VencorContext != nil {
				log.InfofCtx(ctx, " M (Stage): EvaluationContext: %+v", *s.Context.VencorContext.EvaluationContext)
			}
		}
